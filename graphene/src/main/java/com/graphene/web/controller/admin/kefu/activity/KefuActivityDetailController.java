...
public class KefuActivityDetailController {
	...
	public String createActivity(MultipartHttpServletRequest request,@Valid ActivityForm activityForm, 
					...
					for(int i=0;i<files.size();i++){
						...
						if(!files.get(i).isEmpty()){
							if(multipartFile.getSize()>MAX_FILE_SIZE_2M){
								...
								String path = session.getServletContext().getRealPath("/")+"/resources/attached/"+
											userInfo.getId()+"/activity/"+newActivity.getId();
								...
								FileUtil.getInstance().getPreviewImage(saveDest, new File(previewFile), fileExtension
										,GlobalDefs.ANNOUNCEMENT_PHOTO_WIDTH,GlobalDefs.ANNOUNCEMENT_PHOTO_HEIGHT);
								String savePath = FileUtil.getInstance().getSavePath("activity", userInfo.getId(), newActivity.getId()+"", request)
										+"/small"+"."+fileExtension;
	...
	public String editActivity(MultipartHttpServletRequest request,@Valid ActivityForm activityForm, BindingResult result,
			HttpSession session,RedirectAttributes redirectAttributes,@RequestParam("activity_id") Long id){
		UserInfo userInfo = (UserInfo) session.getAttribute(GlobalDefs.SESSION_USER_INFO);
		if(result.hasErrors()){
				...
				for(int i=0;i<files.size();i++){
					...
					if(!files.get(i).isEmpty()){
						if(multipartFile.getSize()>MAX_FILE_SIZE_2M){
							...
							String fileName = multipartFile.getOriginalFilename();
							String fileExtension = fileName.substring(fileName.lastIndexOf(".")+1);
							String path = session.getServletContext().getRealPath("/")+"/resources/attached/"+userInfo.getId()+"/activity/"
										  +activity.getId();						
							FileUtil.getInstance().createRealPath(path, session);
							String previewFile = path+File.separator+"small"+"."+fileExtension;
							File saveDest = new File(path + File.separator + fileName);
							...
							FileUtil.getInstance().getPreviewImage(saveDest, new File(previewFile), fileExtension,
									GlobalDefs.ANNOUNCEMENT_PHOTO_WIDTH,GlobalDefs.ANNOUNCEMENT_PHOTO_HEIGHT);
							String savePath = FileUtil.getInstance().getSavePath("activity", userInfo.getId(), activity.getId()+"", request)
									+"/small"+"."+fileExtension;
