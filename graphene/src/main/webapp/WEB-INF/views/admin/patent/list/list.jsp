		...
		<table class=" blue" id="mytab" cellpadding="4" width=100%  border=0>
			...
			<tbody>
				<c:forEach  items="${page.content}" var="page">
					<tr>
						<td ><a href='<c:url value="/admin/patent/view/${page.id }"></c:url>' >${page.patentNum }</a></td><td >${page.patentName }</td>
						<td >${page.applicationDate }</td>
						<td align="center"><a href='<c:url value="/admin/patent/edit/${page.id}"></c:url>'>修改</a> | 
						<a class="destoryPatentPostBtn" href="#destoryPatentPostModal" role="button" data-toggle="modal" data-target="#destoryPatentPostModal">删除</a><input type="hidden"  value="${page.id}"> </td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
...
<div class="modal hide fade" id="destoryPatentPostModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	  ...
	  <div class="modal-footer">
	    ...
	    <form action='<c:url value="/admin/patent/delete"></c:url>' method="post" style="display: inline-block;" >
	    	<input id="p_delete_id" type="hidden" name="patent_id" />
