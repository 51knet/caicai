 	...
 	height: 24px; font-size:13px; padding: 2px 4px; margin-top: -1px;
		...
		<div class="banner_front custom" >
			...
			<a href="<c:url value='/admin' ></c:url>">
				<c:if test="${userInfo.user.banner_id == null || userInfo.role != 'teacher' }">
					<img src="<c:url value='/resources/img/banner/default_banner.jpg' ></c:url>" />
				</c:if>
				<c:if test="${userInfo.user.banner_id != null }">
					<img src="<c:url value='/resources/img/banner/b${userInfo.user.banner_id}.jpg' ></c:url>" />
				</c:if>
			</a>
		</div>
		<!--/.banner --><!--/span-->
