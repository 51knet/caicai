...
<!-- 
 ...
 <table border="1" cellspacing="0" cellpadding="5">
  		...
  		<c:forEach items="${unReadMsgList}" var = "urm" >
  			<tr>
  				<td>${urm.sendMsg.id}</td>
  				<td>${urm.sendMsg.title}</td>
  				<td>
  					<c:if test="${urm.isRead==1 }">
  						未读
  					</c:if>
  				</td>
  				<td>${urm.sendMsg.date}</td>
  				<td>${urm.sendMsg.user.name}</td>
  				<td><a href='<c:url value="/admin/teacher/message/detailOne?mid=${urm.sendMsg.id}&urmid=${urm.id}"></c:url>'>详细</a> |<a href="#">删除</a></td>
  			</tr>
  		</c:forEach>
	</table>
